...
self.insert = async (table, data) => {
  return self.query(sqlInsert(data, table), data);
};
...
self.update = async (table, data, condition) => {
  return self.query(sqlUpdate(condition, data, table), data);
};
...
self.delete = async (table, condition) => {
  return self.query(sqlRemove(condition, table), condition);
};
...
self.onmessage = async (e) => {
  try {
    ...
    const result = self[e.data.action]
      ? await self[e.data.action].apply(self, args)
      : await self["query"].apply(self, args);
