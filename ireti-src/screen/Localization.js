...
import { onCeateNew, onModalClose, onModalOk } from '../controller/localization';
            ...
            <TabsProvider defaultIndex={screenState.tab} onChangeIndex={() => { }}>
                <Tabs>
                    <TabScreen label="Paises" icon="earth" badge={state.country.data.length} onPress={() => { screenDispatch({ type: 'CHNAGE_TAB', payload: 0 }) }}>
                        <>
                            <Text>Paises</Text>
                            {screenState.tab === 0 && <Country styles={styles} screenDispatch={screenDispatch} countryAttr={countryAttr} setNewCountryData={setNewCountryData} error={errorCountry} setError={setErrorCountry} nameInputRef={countryNameInputRef} />}
                        </>
                    </TabScreen>
                    ...
                    <TabScreen label="Provincias" icon="compass" badge={state.province.data.length} onPress={() => { screenDispatch({ type: 'CHNAGE_TAB', payload: 1 }) }}>
                        <>
                            ...
                            {screenState.tab === 1 && <Province styles={styles} screenDispatch={screenDispatch} provinceAttr={provinceAttr} setNewProvinceData={setNewProvinceData} error={errorProvince} setError={setErrorProvince} nameInputRef={provinceNameInputRef} />}
                        </>
                    </TabScreen>
                </Tabs>
            </TabsProvider>
